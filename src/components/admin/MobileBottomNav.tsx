...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-neutral-200 px-1 pt-1 md:hidden shadow-lg pb-[calc(0.25rem+env(safe-area-inset-bottom,0px))]">
          ...
          return (
            <Link
              ...
              className={`relative flex flex-col items-center justify-center min-h-[48px] py-1.5 px-0.5 rounded-xl transition-all select-none active:scale-95 ${
                isActive
                  ? "bg-brand-900 text-white font-black shadow-sm"
                  : "text-neutral-600 hover:bg-neutral-100 font-bold"
              }`}
            >
              {item.badge !== undefined && (
                <span className="absolute top-1 right-1.5 sm:right-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-600 px-1 text-[10px] font-black text-white ring-2 ring-white">
                  {item.badge}
                </span>
              )}
